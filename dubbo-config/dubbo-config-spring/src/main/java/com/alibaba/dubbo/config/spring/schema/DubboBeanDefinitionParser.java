...
public class DubboBeanDefinitionParser implements BeanDefinitionParser {
    ...
    /**
     * 是否需要 Bean 的 `id` 属性，required ，是否需要在 Bean 对象的编号( id ) 不存在时，自动生成编号。无需被其他应用引用的配置对象，无需自动生成编号。例如有 <dubbo:reference />
     */
    ...
    private static BeanDefinition parse(Element element, ParserContext parserContext, Class<?> beanClass, boolean required) {
         ...
         */
        beanDefinition.setLazyInit(false);// （强调）默认 lazyInit = false
        ...
        if ((id == null || id.length() == 0) && required) {
            ...
            while (parserContext.getRegistry().containsBeanDefinition(id)) {
                id = generatedBeanName + (counter++);// 若 id 已存在，通过自增序列，解决重复
            }
        }
                        ...
                        if (value.length() > 0) {
                            ...
                            if ("registry".equals(property) && RegistryConfig.NO_AVAILABLE.equalsIgnoreCase(value)) {
                                ...
                                if (isPrimitive(type)) {// 兼容性处理
                                    ...
                                    reference = value;
                                } else if ("protocol".equals(property)// 处理在 `<dubbo:provider />` 或者 `<dubbo:service />` 上定义了 `protocol` 属性的 兼容性
            ...
            for (int i = 0; i < nodeList.getLength(); i++) {
                ...
                if (node instanceof Element) {
                    ...
                    if ("method".equals(node.getNodeName()) || "method".equals(node.getLocalName())) {// 这三行，判断值解析 `<dubbo:method />`
                        ...
                        String name = id + "." + methodName;// 添加到 `methods` 中
