...
public class LeastActiveLoadBalance extends AbstractLoadBalance {
    ...
    protected <T> Invoker<T> doSelect(List<Invoker<T>> invokers, URL url, Invocation invocation) {
        ...
        int firstWeight = 0; // Initial value, used for comparision// 是否所有权重相同
        ...
        if (!sameWeight && totalWeight > 0) {
            ...
            for (int i = 0; i < leastCount; i++) {// 并确定随机值落在哪个片断上
        ...
        return invokers.get(leastIndexs[random.nextInt(leastCount)]);// 如果权重相同或权重为0则均等随机
    }
}