...
/**
 * 实现 AbstractClusterInvoker 抽象类，ForkingClusterInvoker 实现类
 */
public class ForkingClusterInvoker<T> extends AbstractClusterInvoker<T> {
    ...
    public Result doInvoke(final Invocation invocation, List<Invoker<T>> invokers, LoadBalance loadbalance) throws RpcException {
        ...
        final AtomicInteger count = new AtomicInteger();// 异常计数器
        final BlockingQueue<Object> ref = new LinkedBlockingQueue<Object>();// 创建阻塞队列
            ...
            executor.execute(new Runnable() {
                public void run() {
                    try {
                        ...
                        int value = count.incrementAndGet();// 异常计数器 + 1
